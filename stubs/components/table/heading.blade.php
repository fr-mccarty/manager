...
<th
    {{ $attributes->merge(['class' => 'px-' . $px . ' py-3 bg-gray-200'])->only(['class', 'title']) }}
>
    @unless ($sortable)
        <span class="text-left text-sm leading-3 font-semibold text-cool-gray-500 uppercase tracking-wider">{{ $slot }}</span>
    @else
        <button {{ $attributes->except('class') }} class="flex items-center space-x-1 text-left text-sm leading-3 font-semibold text-cool-gray-500 uppercase tracking-wider group focus:outline-none focus:underline">
            <span>{{ $slot }}</span>
